...
handlers.authorizeUser = {
    ...
    handler: function(request, reply) {
        ...
        if (!['code', 'token'].includes(responseType)) {
          return reply(Boom.badRequest('Invalid response type. Should be: code or token'))
        }
...
/**
 * Get unauthorization page handler
 * GET /oauth2/unauthorize
 */
handlers.getUnauthorizationView = {
    auth: 'session',
    handler: function(request, reply) {
        reply.view('unauthorization', {
          title: 'Unauthorization page',
          path: request.url.path,
          client_id: request.query.client_id
        });
    }
...
/**
 * Get authorization page handler
 * GET /oauth2/{account}/authenticate
 */
 ...
 * POST /oauth2/{account}/authenticate
 */
    ...
    handler: function(request, reply) {
        request.server.methods.services.act({
            ...
            if (err || !data) {
                ...
                return reply.redirect(`/oauth2/${request.params.account}/authenticate`);
            }
...
handlers.getAccessToken = {
    ...
    handler: function(request, reply) {
      ...
      if (!['authorization_code', 'refresh_token'].includes(grant)) {
        return reply(Boom.badRequest('Invalid grant type. Should be: authorization_code or refresh_token'))
      }
 ...
 handlers.getToken = {
     ...
     handler: function(request, reply) {
       ...
       if (!['client_credentials', 'password'].includes(grant)) {
         return reply(Boom.badRequest('Invalid grant type. Should be: client_credentials or password'))
       }
